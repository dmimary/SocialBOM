    ...
    .controller('ShopsController', function ($scope, $log, $filter, $http, Shops, Items, SharedFactory) {
        ...
        $scope.$watch('factory.current_item', function (newValue, oldValue) {
            if ($scope.factory.current_item) {
                resource.query({
                    'item_id': $scope.factory.current_item._id.$oid
                }, function (resp) {
                    ...
                    $scope.factory.current_item.$update({
                        'id': id
                    }, {
                        "max_cost": $scope.factory.max_item_cost,
                        "max_cost": $scope.factory.min_item_cost
                    });
        ...
        this.new = function () {
            ...
            var extra_json = {
                'item_id': $scope.factory.current_item._id.$oid
            }; // when new and query - Cannot read property '$oid' of undefined
        ...
        $scope.grid_obj.onRegisterApi = function (gridApi) {
            ...
            gridApi.edit.on.afterCellEdit($scope, function (rowEntity, colDef, newValue, oldValue) {
                ...
                //////////////////////////////////////// Aliexpress & Ebay Parsers ////////////////////////////////////
                
                // check that link from aliexpress or ebay
                if ((field === 'link') && (newValue.indexOf("aliexpress.com") !== -1 || newValue.indexOf("ebay.com") !== -1)) {
                    console.log("link from ali"); // display the link in web consol
                    ...
                    $http.get('/parser', {
                        ...
                        // display the name of store in cell "Магазин"
                        rowEntity['name'] = resp.data.name;    
                        // display the price in cell "Стоимость"
                        rowEntity['cost'] = resp.data.price;
                        // display the RUB currency in cell "Валюта"
                        rowEntity['currency'] = 'RUB';
